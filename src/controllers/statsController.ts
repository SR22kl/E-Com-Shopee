...
export const getDashboardStats = TryCatch(async (req, res, next) => {
  ...
  if (myCache.has(key)) stats = JSON.parse(myCache.get(key) as string);
    ...
    const orderMonthCount = Array(6).fill(0);
    const orderMonthRevenue = Array(6).fill(0);
...
export const getBarCharts = TryCatch(async (req, res, next) => {
  ...
  if (myCache.has(key)) charts = JSON.parse(myCache.get(key) as string);
    ...
    const productCounts = getChartData({
      length: 6,
      today,
      docArr: products.map((doc) => doc.toObject()),
    });

    const usersCounts = getChartData({
      length: 6,
      today,
      docArr: users.map((i) => i.toObject()),
    });

    const ordersCounts = getChartData({
      length: 12,
      today,
      docArr: orders.map((i) => i.toObject()),
    });
